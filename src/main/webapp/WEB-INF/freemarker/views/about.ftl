<!DOCTYPE html>
<html lang="zh-CN">
<meta charset="utf-8" name="viewport" content="width=device-width, initial-scale=1">
<head>
    <title>Oliver's Blogs</title>
    <link href="${rc.contextPath}/resources/css/bootstrap.min.css" rel="stylesheet">
    <link href="${rc.contextPath}/resources/css/bootstrap.css" rel="stylesheet">
    <style type="text/css">
        body {
            background-color: #CCC;
        }
    </style>
</head>
<body>
<div class="container">
<#include "./common/header.ftl"/>
    <div class="row"
         style="background-color: white;border-top-left-radius:5px;border-top-right-radius:5px;line-height: 30px;padding: 20px;  ">

        <div class="col-xs-11 col-md-10">
            <address>
                <h3>个人简介</h3>
                <hr>
                <img src="${rc.contextPath}/resources/img/me.jpg"><br><br>
                <strong>Oliver</strong>
                <p>
                    目前在一家知名互联网公司从事java后台和Android App研发工作！
                </p>

            </address>
            <address>
                <strong>个人爱好</strong><br>
                <p>热爱篮球、喜欢登山、跑步、骑行、户外野营</p>

            </address>

            <address>
                <strong> 微博账号</strong><br>
                <a href="http://weibo.com/ningvip/home"><img src="${rc.contextPath}/resources/img/weibo.png" width="20" height="20">&nbsp;猫仔仔</a>
                <br>
                <strong>Motto</strong><br>
                <p>keep calm and carry on!</p>
                <br>

            </address>

            <address>
                <strong>QQ:</strong><br>
                <a href="${rc.contextPath}#"><img src="${rc.contextPath}/resources/img/qq.png" width="20" height="20">&nbsp;1066114096</a>
            </address>

            <address>
                <strong>Email:</strong><br>
                <a href="${rc.contextPath}${rc.contextPath}mailto:#">lxnlhr@163.com</a>
            </address>


        </div>

    </div>
<#include "./common/footer.ftl" />
</div>

</body>
</html>